document.addEventListener('DOMContentLoaded', function() {
  ...
  document.addEventListener('click', event => {
    ...
    if(element.id === 'archive_email'){
      ...
      const data_reply={"sender":element.dataset.sender,
...
function load_mailbox(mailbox) {
 ...
 // elem.parentNode.removeChild(elem);
  ...
  if(mailbox === 'inbox'){
    ...
    document.querySelector('#sent-view').innerHTML = `<h3>${mailbox.charAt(0).toUpperCase() + mailbox.slice(1)}</h3>`;
   ...
   // alert("ola");
    ...
    document.querySelector('#archived-view').innerHTML = `<h3>${mailbox.charAt(0).toUpperCase() + mailbox.slice(1)}</h3>`;
